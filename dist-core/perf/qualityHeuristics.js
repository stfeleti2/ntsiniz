...
exports.buildQualityConfig = void 0;
exports.classifyDeviceTier = classifyDeviceTier;
exports.initialQualityForTier = initialQualityForTier;
exports.getQualityConfig = getQualityConfig;
exports.shouldDegrade = shouldDegrade;
exports.shouldUpgrade = shouldUpgrade;
exports.degradeMode = degradeMode;
exports.upgradeMode = upgradeMode;
 ...
 */
function classifyDeviceTier(input) {
...
function initialQualityForTier(tier) {
    if (tier === 'HIGH')
        return 'HIGH';
    if (tier === 'MID')
        return 'BALANCED';
...
function getQualityConfig(mode, tier) {
    // AUTO resolves to a concrete mode, but we keep mode label for display.
    const resolved = mode === 'AUTO' ? initialQualityForTier(tier) : mode;
    if (resolved === 'HIGH') {
        return {
            mode,
            tier,
...
function shouldDegrade(arg1, arg2) {
    const usingSnapshot = typeof arg1 === 'object';
    const s = usingSnapshot ? arg1 : (0, perfMonitor_1.getPerfSnapshot)();
    const currentResolvedMode = usingSnapshot ? arg2 : arg1;
    if ((s.frameBusDropped ?? 0) > 0)
        return true;
    // Compare against targets for current mode.
    const tier = 'MID'; // doesn't matter; targets keyed by mode below
    const cfg = getQualityConfig(currentResolvedMode, tier);
    if (s.p95StallMs >= cfg.stallP95TargetMs)
        return true;
    if (s.worstStallMs >= cfg.stallWorstTargetMs)
        return true;
    return false;
}
function shouldUpgrade(arg1, currentResolvedMode) {
    const usingSnapshot = typeof arg1 === 'object';
    ...
    if (s.p95StallMs > p95Limit)
        return false;
    if (s.worstStallMs > cfg.stallWorstTargetMs * 0.75)
        return false;
    return stableWindowMs >= 120_000;
}
// Back-compat helpers used by tests and older call sites.
exports.buildQualityConfig = getQualityConfig;
function degradeMode(current) {
    return current === 'HIGH' ? 'BALANCED' : 'LITE';
}
function upgradeMode(current, target) {
    if (target) {
