...
async function runDrillWithDrivers(ctx, drivers, opts) {
    ...
    const dsp = (0, voiceDsp_1.createVoiceDspSession)({
        sampleRate,
    ...
    try {
        mic = await drivers.startMic({ sampleRate, frameDurationMs: frameMs }, (ev) => {
            ...
            bus.push(ev, (frame) => {
                ...
                try {
                    const diag = !reading
                        ? {
                            reason: dspOut.signalQuality === 'poor' || (frameRms != null && frameRms < noiseGate * 1.2)
                                ? 'too_quiet'
                                : 'low_confidence',
                            frameRms,
                            framePeak,
        ...
        bus.stop();
        await dsp.close().catch((e) => logger_1.logger.warn('dsp close failed', e));
