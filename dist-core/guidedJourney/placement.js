...
exports.estimateLikelyFamily = estimateLikelyFamily;
exports.buildFirstWinSnapshot = buildFirstWinSnapshot;
...
function estimateLikelyFamily(pitchBand) {
    if (pitchBand.lowMidi == null || pitchBand.highMidi == null)
    ...
    if (span < 3)
        return { label: null, confidence: 0.18 };
    if (center >= 67)
        return { label: 'Likely lighter / higher voice', confidence: 0.68 };
    if (center <= 57)
        return { label: 'Likely fuller / lower voice', confidence: 0.68 };
    return { label: 'Likely mid-range voice', confidence: 0.62 };
}
function buildFirstWinSnapshot(input) {
