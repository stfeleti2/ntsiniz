...
exports.decodeWavWaveform = decodeWavWaveform;
exports.decodeWavSamples = decodeWavSamples;
 ...
 */
async function decodeWavWaveform(uri, bars = 72) {
    if (!uri)
        return null;
    let FileSystem;
    try {
 ...
 */
async function decodeWavSamples(uri) {
    if (!uri)
        return null;
    let FileSystem;
    try {
...
function ascii(bytes, off, len) {
    let s = '';
    for (let i = 0; i < len; i++)
        s += String.fromCharCode(bytes[off + i] ?? 0);
    return s;
}
function findWavDataChunk(dv, bytes) {
...
function downsamplePeaksFromI16(dv, dataOffset, numSamples, bars) {
    if (bars <= 0)
        return [];
    const out = [];
    if (numSamples <= 0)
