import type { NeumorphismRule } from './types'

export const neumorphismRules: NeumorphismRule[] = [
...
export function getNeumorphismRule(variant: NeumorphismRule['variant'], state: NeumorphismRule['state']) {
  return neumorphismRules.find((rule) => rule.variant === variant && rule.state === state) ??
    neumorphismRules.find((rule) => rule.variant === variant && rule.state === 'default') ??
    neumorphismRules[0]
}
