import { listFeedPostsWithStats } from './postsRepo'
import { listFollowingIds } from './followsRepo'
import type { DiscoverFilter, FeedPostWithStats } from './feedTypes'
...
export async function listJourneyFeed(limit = 80): Promise<FeedPostWithStats[]> {
  const all = await listFeedPostsWithStats(Math.max(limit, 120))
  return all.filter((p) => p.type === 'performance').slice(0, limit)
}

export async function listChallengeFeed(limit = 80): Promise<FeedPostWithStats[]> {
  const all = await listFeedPostsWithStats(Math.max(limit, 120))
  return all.filter((p) => p.type === 'challenge').slice(0, limit)
}

export async function listDiscoverFeed(filter: DiscoverFilter, limit = 80, viewerId?: string): Promise<FeedPostWithStats[]> {
  const all = await listFeedPostsWithStats(240)

  const engagement = (p: FeedPostWithStats) => p.stats.reactions * 2 + p.stats.comments * 3
  const recencyBoost = (createdAt: number) => {
    // 0..1.5 boost over ~14 days
    const ageDays = (Date.now() - createdAt) / 86400000
    return Math.max(0, 1.5 - ageDays / 10)
  }

  const diversifyByAuthor = (items: FeedPostWithStats[], maxPerAuthor = 2) => {
  ...
  if (filter === 'forYou') {
    ...
    const following = viewerId ? await listFollowingIds(viewerId).catch(() => []) : []
  ...
  // newCreators
  const counts = new Map<string, number>()
  for (const p of all) counts.set(p.authorId, (counts.get(p.authorId) ?? 0) + 1)
  return all
