import type { DrillPack, Drill } from "../drills/schema"
import type { VoiceProfile } from "../storage/profileRepo"

export function pickNextDrill(
  pack: DrillPack,
  profile: VoiceProfile,
  opts?: { lastDrillId?: string; lastScore?: number; focusType?: Drill["type"] },
) {
  ...
  const cycle: Drill["type"][] = ["match_note", "sustain", "slide", "interval", "melody_echo"]
...
function correctiveType(p: VoiceProfile): Drill["type"] {
  if (p.voicedRatio < 0.45) return "sustain"
  if (p.wobbleCents > 18) return "sustain"
  if (Math.abs(p.biasCents) > 18) return "match_note"
  if (p.overshootRate > 0.2) return "slide"
  if (Math.abs(p.driftCentsPerSec) > 4) return "sustain"
  return "match_note"
}
