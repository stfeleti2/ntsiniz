 ...
 */
export function buildScoreBreakdown(metrics: any, overallScore: number): ScoreBreakdown {
  const m = metrics ?? {}

  const clamp = (x: number) => Math.max(0, Math.min(100, x))
  const inv = (x: number, worst: number) => clamp(100 - (x / worst) * 100)
  ...
  const notes: string[] = []
  if (typeof m.avgAbsCents === 'number') notes.push(`Pitch: ~${Math.round(m.avgAbsCents)}c off on average`)
  if (typeof m.wobbleCents === 'number') notes.push(`Stability: ~${Math.round(m.wobbleCents)}c wobble`)
  if (typeof m.timeToEnterMs === 'number') notes.push(`Timing: locked in ${(m.timeToEnterMs / 1000).toFixed(1)}s`)
  if (typeof m.voicedRatio === 'number') notes.push(`Confidence: ${Math.round(m.voicedRatio * 100)}% voiced`)
