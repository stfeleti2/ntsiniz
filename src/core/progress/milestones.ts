import type { Milestones, MilestonePoint, SessionAggregate } from "./types"
...
function pickClosest(aggs: SessionAggregate[], targetDayMs: number, windowDays = 2): SessionAggregate[] {
  ...
  let best: SessionAggregate | null = null
...
function asPoint(label: string, rows: SessionAggregate[]): MilestonePoint | null {
...
export function computeMilestones(aggs: SessionAggregate[]): Milestones {
  const xs = [...aggs]
...
export function formatMilestone(point: MilestonePoint | null) {
  if (!point) return "—"
