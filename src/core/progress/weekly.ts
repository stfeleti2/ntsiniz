...
import type { SessionAggregate, WeeklyReport } from "./types"
...
export function computeDailyAverages(aggs: SessionAggregate[], startMs: number, endMs: number) {
  ...
  const days: { dayMs: number; avgScore: number }[] = []
  for (let d = startOfDayMs(startMs); d < endMs; d = addDaysMs(d, 1)) {
    const arr = map.get(d) ?? []
    if (arr.length) days.push({ dayMs: d, avgScore: Math.round(avg(arr)) })
  }
  return days
}

function avgScoreFromAttempts(attempts: Attempt[]) {
  if (!attempts.length) return null
  return avg(attempts.map((a) => a.score))
}

function avgWobbleFromAttempts(attempts: Attempt[]) {
  const xs = attempts
    .map((a) => (a as any)?.metrics?.wobbleCents)
    .filter((v) => typeof v === "number" && Number.isFinite(v)) as number[]
  if (!xs.length) return null
  return avg(xs)
}

function avgMetric(attempts: Attempt[], pick: (a: Attempt) => number | null | undefined) {
  const xs = attempts
    .map((a) => pick(a))
    .filter((v) => typeof v === "number" && Number.isFinite(v)) as number[]
  if (!xs.length) return null
...
function groupAvg(atts: Attempt[], keyOf: (a: Attempt) => string) {
  const m = new Map<string, { scores: number[]; n: number }>()
  for (const a of atts) {
    const key = keyOf(a)
  ...
  const out = new Map<string, { avgScore: number; attempts: number }>()
  for (const [k, v] of m.entries()) out.set(k, { avgScore: avg(v.scores), attempts: v.n })
  return out
}

function computeStreaks(dayMsSorted: number[]) {
...
function mostImprovedDrill(attemptsThis: Attempt[], attemptsPrev: Attempt[]) {
  const group = (atts: Attempt[]) => {
    ...
    for (const [k, v] of m.entries()) out.set(k, avg(v))
    return out
  }
  ...
  if (bestDelta == null || bestLabel == null) return { label: null, delta: null }
...
function generateInsight(input: {
  ...
  activeDays: number
  sessions: number
  accuracyDeltaCents: number | null
  wobbleDeltaCents: number | null
  voicedDeltaRatio: number | null
...
export function computeWeeklyReport(params: {
  weekStartMs: number
  ...
  for (const s of sessionsInWeek) activeDaySet.add(startOfDayMs(s.startedAt))
  ...
  const wobbleDelta =
    wobbleAvg == null || prevWobbleAvg == null ? null : Number((prevWobbleAvg - wobbleAvg).toFixed(1))

  // extra proof metrics (accuracy/stability/speed/voice)
  const accAvg = avgMetric(attempts, (a) => (a as any)?.metrics?.avgAbsCents)
  const prevAccAvg = avgMetric(prevAttempts, (a) => (a as any)?.metrics?.avgAbsCents)
  const accDelta = accAvg == null || prevAccAvg == null ? null : Number((prevAccAvg - accAvg).toFixed(1))

  const voicedAvg = avgMetric(attempts, (a) => (a as any)?.metrics?.voicedRatio)
  const prevVoicedAvg = avgMetric(prevAttempts, (a) => (a as any)?.metrics?.voicedRatio)
  const voicedDelta =
    voicedAvg == null || prevVoicedAvg == null ? null : Number((voicedAvg - prevVoicedAvg).toFixed(3))

  const confAvg = avgMetric(attempts, (a) => (a as any)?.metrics?.confidenceAvg)
  const prevConfAvg = avgMetric(prevAttempts, (a) => (a as any)?.metrics?.confidenceAvg)
  const confDelta = confAvg == null || prevConfAvg == null ? null : Number((confAvg - prevConfAvg).toFixed(3))

  const speedAvg = avgMetric(attempts, (a) => (a as any)?.metrics?.timeToEnterMs)
  const prevSpeedAvg = avgMetric(prevAttempts, (a) => (a as any)?.metrics?.timeToEnterMs)
  ...
  const typeThis = groupAvg(attempts, (a) => String((a as any)?.metrics?.drillType ?? "unknown"))
  const typePrev = groupAvg(prevAttempts, (a) => String((a as any)?.metrics?.drillType ?? "unknown"))
